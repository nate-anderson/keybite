...
func TestNewBucketDriver(t *testing.T) {
	...
	_, err = driver.NewBucketDriver(pageExtension, bucketName, accessKeyID, accessKeySecret, "")
	util.Ok(t, err)
}
...
func TestBucketCreateAutoIndex(t *testing.T) {
	...
	bd, err := driver.NewBucketDriver(pageExtension, bucketName, accessKeyID, accessKeySecret, "")
	util.Ok(t, err)
