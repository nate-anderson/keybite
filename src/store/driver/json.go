...
// JSONDriver enables writing and reading indexes from local filesystem
...
// jsonAutoPage is an intermediate struct used for writing structured json data of an auto index page
type jsonAutoPage struct {
	Data        *map[uint64]string `json:"data"`
	OrderedKeys *[]uint64          `json:"orderedKeys"`
}

// jsonMapPage is an intermediate struct used for writing structured json data of a map index page
type jsonMapPage struct {
	Data        *map[string]string `json:"data"`
	OrderedKeys *[]string          `json:"orderedKeys"`
}

// NewJSONDriver instantiates a new filesystem storage driver
func NewJSONDriver(dataDir string, pageExtension string, lockDuration time.Duration) (JSONDriver, error) {
	if _, err := os.Stat(dataDir); os.IsNotExist(err) {
		return JSONDriver{}, ErrDataDirNotExist(dataDir, err)
	}
...
func (d JSONDriver) ReadPage(fileName string, indexName string, pageSize int) (map[uint64]string, []uint64, error) {
...
func (d JSONDriver) ReadMapPage(fileName string, indexName string, pageSize int) (map[string]string, []string, error) {
...
func (d JSONDriver) WritePage(vals map[uint64]string, orderedKeys []uint64, filename string, indexName string) error {
	filePath := path.Join(d.dataDir, indexName, AddSuffixIfNotExist(filename, d.pageExtension))
...
func (d JSONDriver) WriteMapPage(vals map[string]string, orderedKeys []string, fileName string, indexName string) error {
	filePath := path.Join(d.dataDir, indexName, AddSuffixIfNotExist(fileName, d.pageExtension))
	file, err := os.OpenFile(filePath, os.O_RDWR, 0755)
	if err != nil {
		if os.IsNotExist(err) {
...
func (d JSONDriver) ListPages(indexName string) ([]string, error) {
...
func (d JSONDriver) CreateAutoIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
	return os.Mkdir(indexPath, 0755)
}

// CreateMapIndex creates the folder for a map index in the data dir
func (d JSONDriver) CreateMapIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
	return os.Mkdir(indexPath, 0755)
}

// LockIndex creates a lockfile in the specified index
func (d JSONDriver) LockIndex(indexName string) error {
...
func (d JSONDriver) UnlockIndex(indexName string) error {
...
func (d JSONDriver) IndexIsLocked(indexName string) (bool, time.Time, error) {
...
func (d JSONDriver) DropAutoIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
...
func (d JSONDriver) DropMapIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
	if _, err := os.Stat(indexPath); os.IsNotExist(err) {
		return fmt.Errorf("failed dropping index '%s': directory does not exist", indexName)
	}
