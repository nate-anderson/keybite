...
// GobDriver enables writing and reading indexes from local filesystem
...
// NewGobDriver instantiates a new filesystem storage driver
func NewGobDriver(dataDir string, pageExtension string, lockDuration time.Duration) (GobDriver, error) {
	if _, err := os.Stat(dataDir); os.IsNotExist(err) {
		return GobDriver{}, ErrDataDirNotExist(dataDir, err)
	}
...
func (d GobDriver) ReadPage(fileName string, indexName string, pageSize int) (map[uint64]string, []uint64, error) {
	...
	decoder := gob.NewDecoder(pageFile)
...
func (d GobDriver) ReadMapPage(fileName string, indexName string, pageSize int) (map[string]string, []string, error) {
...
func (d GobDriver) WritePage(vals map[uint64]string, orderedKeys []uint64, filename string, indexName string) error {
	filePath := path.Join(d.dataDir, indexName, AddSuffixIfNotExist(filename, d.pageExtension))
...
func (d GobDriver) WriteMapPage(vals map[string]string, orderedKeys []string, fileName string, indexName string) error {
	filePath := path.Join(d.dataDir, indexName, AddSuffixIfNotExist(fileName, d.pageExtension))
	file, err := os.OpenFile(filePath, os.O_RDWR, 0755)
	if err != nil {
		if os.IsNotExist(err) {
...
func (d GobDriver) ListPages(indexName string) ([]string, error) {
...
func (d GobDriver) CreateAutoIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
	return os.Mkdir(indexPath, 0755)
}

// CreateMapIndex creates the folder for a map index in the data dir
func (d GobDriver) CreateMapIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
	return os.Mkdir(indexPath, 0755)
}

// LockIndex creates a lockfile in the specified index
func (d GobDriver) LockIndex(indexName string) error {
...
func (d GobDriver) UnlockIndex(indexName string) error {
...
func (d GobDriver) IndexIsLocked(indexName string) (bool, time.Time, error) {
...
func (d GobDriver) DropAutoIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
...
func (d GobDriver) DropMapIndex(indexName string) error {
	indexPath := path.Join(d.dataDir, indexName)
	if _, err := os.Stat(indexPath); os.IsNotExist(err) {
		return fmt.Errorf("failed dropping index '%s': directory does not exist", indexName)
	}
